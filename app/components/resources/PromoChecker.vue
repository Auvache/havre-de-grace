<!--
  ...
  content/resources/promo-checker.yml. This component asks and adds up.
...
<template>
  <div class="space-y-10">
    <!-- --- Mode ------------------------------------------------------- -->
    <section aria-labelledby="rs-mode-heading" class="space-y-4">
      <h2 id="rs-mode-heading" class="label-text">What are you checking?</h2>

      <div class="grid gap-3 sm:grid-cols-2" role="radiogroup" aria-labelledby="rs-mode-heading">
        <button
        ...
        >
          <span class="rs-mode__label">{{ mode.label }}</span>
    ...
    <!-- --- Questions --------------------------------------------------- -->
    <section v-if="activeMode" aria-labelledby="rs-questions-heading" class="space-y-5">
      <div class="flex flex-wrap items-baseline justify-between gap-3">
        <h2 id="rs-questions-heading" class="rs-heading">
      ...
      >
        <legend class="rs-question">{{ question.question }}</legend>
        ...
        <p v-if="isFlagged(question)" class="rs-why">
          {{ question.why }}
        </p>
      </fieldset>

      <div class="rs-no-print flex flex-wrap gap-3">
        <button type="button" class="rs-button" @click="clearAnswers">
          Clear answers
        </button>
      </div>
    </section>
    ...
    <section
      ...
      aria-labelledby="rs-result-heading"
      aria-live="polite"
      class="space-y-6"
    >
      <div class="rs-result" :data-level="level">
        <p class="label-text">Reading</p>
        <p id="rs-result-heading" class="rs-result__level">{{ LEVEL_LABEL[level] }}</p>
      ...
      <section v-if="triggered.length" aria-labelledby="rs-flags-heading" class="space-y-4">
        <h3 id="rs-flags-heading" class="rs-heading">
          What you flagged
        </h3>
        <ul class="space-y-3">
      ...
      <section aria-labelledby="rs-next-heading" class="space-y-4">
        <h3 id="rs-next-heading" class="rs-heading">What to do next</h3>
        <ol class="rs-steps">
          <li v-for="(step, index) in nextSteps" :key="index">{{ step }}</li>
        </ol>
      </section>

      <p class="rs-heuristic" role="note">
...
const { state } = useToolStorage<CheckerState>('promo-checker', emptyState, revive)
...
.rs-result[data-level="low"] {
  border-left-color: var(--color-accent);
}

.rs-result[data-level="caution"] {
  border-left-color: #c58a2e;
}

.rs-result[data-level="high"] {
